    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float GGX_D(float roughness, float NdotH)
            {
                ...
                float denominator = UNITY_PI * pow((pow(NdotH, 2) * (pow(roughness, 2) - 1) + 1), 2);
                return numerator / denominator;
            }

            float CookTorrence_G (float NdotL, float NdotV, float VdotH, float NdotH){
                //// TODO: your implementation
                float second = 2 * NdotH * NdotV / VdotH;
                float third = 2 * NdotH * NdotL / VdotH;
                return min(1, min(second, third));
            }
