...
Shader "GoogleVR/Reticle" {
  ...
  SubShader {
    ...
    Pass {
      ...
      fragmentInput vert(vertexInput i) {
        float scale = lerp(0.055 * _DistanceInMeters, 0.04 * _DistanceInMeters, i.vertex.z);
        float3 vert_out = float3(i.vertex.x * scale, i.vertex.y * scale, _DistanceInMeters);

        float2 direction = normalize(vert_out.xy - float2(0, 0));
        float2 boundaryDirection = float2(0, 1);
        float realAngle = degrees(acos(dot(direction, boundaryDirection)));
        if (direction.x < 0) {
            realAngle = 360 - realAngle;
        }
        if (realAngle >= _Angle) {
            vert_out = float3(0, 0, _DistanceInMeters);
        }
